...
app.post('/api/search', async (req, res) => {
  ...
  try {
    ...
    const outputFilePath = path.join(outputDir, 'all-search-results.json');
    ...
    // -------- Persist ALL results cumulatively --------
    ...
    if (fs.existsSync(outputFilePath)) {
      try {
        ...
        console.error('Failed to read existing results file:', err);
      }
    }

    // Append new results
    const combinedResults = existingResults.concat(allResults);
    ...
    console.log(
      `Stored ${allResults.length} new results. Total stored: ${combinedResults.length}`
    );


    res.json({
      totalResults: allResults.length,
      agreementAssetsResults: allResults
    });
  } catch (error) {
